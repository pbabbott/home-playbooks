...
This directory manages VM lifecycle on Proxmox (create, update, destroy). Configuration inside VMs is handled by Ansible; see the [root README](../README.md) for the overall architecture.
...
- First-time setup (API token, tfvars, template, `terraform init`)
- Common commands (`plan`, `apply`, `destroy`, `output`, `fmt`, `validate`)
- Typical workflows and state notes
